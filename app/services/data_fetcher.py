...
# Cache duration: skip stock if updated less than this many hours ago
CACHE_HOURS = 2


# ============================================
# INDICATOR FUNCTIONS (invariate dall'originale)
# ============================================
...
def detect_setup_type(data):
    ...
    poc = data.get("poc_price")
    va_high = data.get("va_high")
    if poc and price and abs(price - poc) / price * 100 <= 3:
        return "pullback_to_poc"
    if va_high and price and price > va_high and rsi < 70:
        return "breakout"
    if ema20 > 0 and ema50 > 0 and price > ema50:
        if abs(price - ema20) / price * 100 <= 2:
            return "ema_bounce"
    if rsi < 35:
...
def detect_fvg(df):
    ...
    for i in range(2, len(df)):
        if low[i] > high[i-2]:
            ...
            fvgs.append({"type": "bullish", "top": round(fvg_top, 2), "bottom": round(fvg_bottom, 2), "size": round(fvg_size, 2), "midpoint": round(mid_price, 2), "filled": filled, "bar_index": i})
        elif high[i] < low[i-2]:
            ...
            fvgs.append({"type": "bearish", "top": round(fvg_top, 2), "bottom": round(fvg_bottom, 2), "size": round(fvg_size, 2), "midpoint": round(mid_price, 2), "filled": filled, "bar_index": i})
...
def detect_wyckoff_phase(df):
    ...
    if price_range_20 < 10 and vol_change < -10 and higher_lows:
        phase = "accumulation"
        confidence = 70 + (10 if vol_change < -20 else 0)
        description = "Tight range with declining volume."
        signal = "bullish_soon"
...
def calc_accumulation_score(df, poc_price, va_low, va_high):
    ...
    if poc_price and current_price < poc_price:
        ...
        if dist <= 5:
            score += 25
            factors.append({"name": "Below POC", "score": 25, "detail": "{}% below".format(round(dist, 1)), "pass": True})
        elif dist <= 15:
            score += 15
            factors.append({"name": "Below POC", "score": 15, "detail": "{}% below".format(round(dist, 1)), "pass": True})
        else:
            factors.append({"name": "Below POC", "score": 0, "detail": "too far", "pass": False})
    else:
        factors.append({"name": "Below POC", "score": 0, "detail": "above POC", "pass": False})
    if va_low and current_price <= va_low * 1.02:
        score += 20
        factors.append({"name": "Near VA Low", "score": 20, "detail": "near/below", "pass": True})
    else:
        factors.append({"name": "Near VA Low", "score": 0, "detail": "above", "pass": False})
    if len(volume) >= 20:
        ...
        if v1 > 0:
            ...
            if vd < -15:
                score += 15
                factors.append({"name": "Vol Decreasing", "score": 15, "detail": "down {}%".format(round(vd)), "pass": True})
            else:
                factors.append({"name": "Vol Decreasing", "score": 0, "detail": "{}%".format(round(vd)), "pass": False})
    score = min(score, 100)
...
def detect_candlestick_patterns(df):
    ...
    if body_pct < 0.15:
        if lower_shadow > body * 2 and upper_shadow > body * 2:
            patterns.append({"name": "Doji", "type": "neutral", "strength": "moderate", "description": "Indecision"})
    if is_bearish1 and is_bearish(body2 > 0) if False else False:
        pass
    if len(df) >= 3:
...
def get_pattern_score_bonus(patterns):
    ...
    for p in patterns:
        if p["type"] == "bullish":
            bonus += 5 if p["strength"] == "strong" else 3
        elif p["type"] == "bearish":
            bonus -= 3 if p["strength"] == "strong" else 1
    return max(-10, min(15, bonus))


# ============================================
# FETCH FUNCTIONS (ottimizzate)
# ============================================

async def fetch_bars(client, symbol):
    """Fetch historical bars per un singolo symbol (per indicatori tecnici)."""
    end = datetime.utcnow()
    start = end - timedelta(days=400)
    url = "{}/v2/stocks/{}/bars".format(ALPACA_DATA_URL, symbol)
    params = {
        "timeframe": "1Day",
...
async def fetch_bars_batch(client, symbols, max_concurrent=5):
    """Fetch bars per piu' symbols in parallelo con semaforo."""
    semaphore = asyncio.Semaphore(max_concurrent)
    results = {}

    async def _fetch_one(sym):
        async with semaphore:
            df = await fetch_bars(client, sym)
            results[sym] = df
...
async def is_fresh(db, collection, key_field, key_value, hours=CACHE_HOURS):
    """Controlla se un documento e' stato aggiornato di recente."""
...
def analyze_stock(ticker, df, sector_code, sector_scores):
    """Analizza una singola stock (calcoli CPU — no async)."""
    if df is None or len(df) < 20:
        return None
    ...
    return {
        ...
        "fvg": fvgs, "wyckoff": wyckoff, "accumulation": accumulation,
        "price_history": price_history, "pattern_bonus": pattern_bonus,
...
# ============================================
# MAIN FUNCTIONS (ottimizzate con batch + cache)
# ============================================

async def fetch_and_analyze_sectors(force=False):
    """Aggiorna dati degli 11 settori SPDR + indici market regime."""
    db = get_db()
    t_start = time.time()
    print("=" * 50)
    print("STARTING SECTORS REFRESH (Optimized)")
    print("=" * 50)
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if spy_df is not None and len(spy_df) >= 20:
            ...
            await db.market_regime.update_one(
                ...
                {"$set": {"symbol": "SPY", "price": round(spy_price, 2), "change_pct": round(spy_change, 2),
                          "ema20": round(spy_ema20, 2), "ema50": round(spy_ema50, 2),
                          "rsi": round(spy_rsi_val, 1), "return_20d": round(spy_return, 2),
                          "updated_at": datetime.utcnow()}},
                upsert=True)
            print("  SPY: ${:.2f} RSI={:.1f} ret20d={:.2f}%".format(spy_price, spy_rsi_val, spy_return))

        # ----------------------------
        # Fetch indices IN PARALLEL (QQQ, IWM, DIA)
        # ----------------------------
        indices_bars = await fetch_bars_batch(client, ["QQQ", "IWM", "DIA"], max_concurrent=3)
        for idx_sym, idx_df in indices_bars.items():
            if idx_df is not None and len(idx_df) >= 2:
                ...
                await db.market_regime.update_one(
                    {"symbol": idx_sym},
                    {"$set": {"symbol": idx_sym, "price": round(ip, 2), "change_pct": ic,
                              "return_20d": ir20, "updated_at": datetime.utcnow()}},
                    upsert=True)
                print("  {}: ${:.2f} ({:+.2f}%)".format(idx_sym, ip, ic))
        ...
        for crypto in ["BTC/USD", "ETH/USD"]:
            try:
                cr = await client.get("{}/v1beta3/crypto/us/bars".format(ALPACA_DATA_URL),
                ...
                if cr.status_code == 200:
                    ...
                    if cbars and len(cbars) >= 2:
                        ...
                        await db.market_regime.update_one(
                            {"symbol": crypto},
                            {"$set": {"symbol": crypto, "price": round(cp, 2), "change_pct": cc,
                                      "updated_at": datetime.utcnow()}},
                            upsert=True)
                        print("  {}: ${:.2f} ({:+.2f}%)".format(crypto, cp, cc))
            except Exception as e:
                print("  {} error: {}".format(crypto, e))

        # ----------------------------
        # Fetch FX proxies IN PARALLEL
        # ----------------------------
        fx_bars = await fetch_bars_batch(client, ["FXE", "UUP"], max_concurrent=2)
        for fx, fx_df in fx_bars.items():
            if fx_df is not None and len(fx_df) >= 2:
                ...
                await db.market_regime.update_one(
                    {"symbol": fx},
                    {"$set": {"symbol": fx, "price": round(fp, 2), "change_pct": fc,
                              "updated_at": datetime.utcnow()}},
                    upsert=True)
                print("  {}: ${:.2f} ({:+.2f}%)".format(fx, fp, fc))

        # ----------------------------
        # Fetch 11 sectors IN PARALLEL
        # ----------------------------
        print("\n  Fetching 11 sector ETFs in parallel...")
        sector_bars = await fetch_bars_batch(client, list(SECTOR_MAP.keys()), max_concurrent=5)

        results = []
        for etf, name in SECTOR_MAP.items():
            try:
                df = sector_bars.get(etf)
                if df is None or len(df) < 20:
                    print("  SKIP {}".format(etf))
                    continue
                close = df["Close"]
                volume = df["Volume"]
                ...
                for idx in range(max(20, len(df) - 90), len(df)):
                    ...
                    history.append({
                        "date": df["datetime"].iloc[idx].strftime("%Y-%m-%d") if "datetime" in df.columns else "d{}".format(idx),
                        "close": round(dc, 2), "rsi": round(dr, 1),
                        "ema10": round(float(e10s.iloc[idx]), 2), "ema20": round(float(e20s.iloc[idx]), 2),
                        "ema50": round(float(e50s.iloc[idx]), 2), "zone": zone,
                    })

                sector_doc = {
                    "code": etf, "name": name, "etf_ticker": etf,
                    "price": round(price, 2), "return_20d": round(float(ret_20d), 2),
                    "strength_score": strength, "trend_score": trend,
                    "volume_score": round(rel_vol * 30, 2), "rsi": rsi,
                    "composite_score": composite, "history": history,
                    "updated_at": datetime.utcnow(),
                }
                ...
                print("  ERROR {}: {}".format(etf, e))
                traceback.print_exc()

    elapsed = round(time.time() - t_start, 1)
    print("\nSECTORS DONE: {}/11 in {}s".format(len(results), elapsed))
    return results


async def fetch_and_analyze_stocks(force=False):
    """Aggiorna le 220 stock con batch fetching e caching."""
    db = get_db()
    t_start = time.time()
    print("=" * 50)
    print("STARTING STOCKS REFRESH (Optimized + Cache)")
    print("=" * 50)
    ...
    # ----------------------------
    # CACHE CHECK: skip fresh stocks
    # ----------------------------
    tickers_to_update = []
    if not force:
        for ticker in all_tickers:
            fresh = await is_fresh(db, "assets", "ticker", ticker, hours=CACHE_HOURS)
            if not fresh:
                tickers_to_update.append(ticker)
        skipped = len(all_tickers) - len(tickers_to_update)
        if skipped > 0:
            print("  Cache: skipping {} fresh stocks (updated <{}h ago)".format(skipped, CACHE_HOURS))
    else:
        tickers_to_update = all_tickers
        print("  Force mode: refreshing ALL {} stocks".format(len(all_tickers)))

    if not tickers_to_update:
        print("  All stocks are fresh! Nothing to update.")
        return []

    print("  Updating {} stocks...".format(len(tickers_to_update)))

    # ----------------------------
    # BATCH FETCH: parallel with semaphore
    # ----------------------------
    results = []
    batch_size = 10  # Fetch 10 stocks in parallel at a time

    async with httpx.AsyncClient(timeout=30) as client:
        for batch_num in range(0, len(tickers_to_update), batch_size):
            batch = tickers_to_update[batch_num:batch_num + batch_size]
            ...
            t_batch = time.time()
            bars_map = await fetch_bars_batch(client, batch, max_concurrent=5)
            ...
            print("    → {} OK, {} skipped in {}s".format(success, skipped_batch, batch_time))

            # Small delay between batches to be nice to API
            if batch_num + batch_size < len(tickers_to_update):
                await asyncio.sleep(0.5)

    elapsed = round(time.time() - t_start, 1)
    print("\n" + "=" * 50)
    print("STOCKS DONE: {}/{} updated in {}s".format(len(results), len(tickers_to_update), elapsed))
    print("  Average: {:.1f}s per stock".format(elapsed / len(results) if results else 0))
    print("=" * 50)
    return results
