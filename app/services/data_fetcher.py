...
async def fetch_fmp_batch(client, symbols):
    result = {}
    from_date = (datetime.now() - timedelta(days=100)).strftime("%Y-%m-%d")
    to_date = datetime.now().strftime("%Y-%m-%d")
...
async def fetch_and_analyze_sectors():
    ...
    if not settings.FMP_API_KEY:
        ...
        return []
    print("=" * 50)
    print("STARTING SECTOR REFRESH (FMP)")
    print("=" * 50)
    async with httpx.AsyncClient(timeout=60, follow_redirects=True) as client:
        spy_df = await fetch_fmp(client, "SPY")
        spy_return = 0
        ...
        etf_list = list(SECTOR_MAP.keys())
        print(f"Fetching sectors: {etf_list}")
        sector_data = await fetch_fmp_batch(client, etf_list)
        print(f"Got data for: {list(sector_data.keys())}")
    results = []
    for etf, name in SECTOR_MAP.items():
        try:
            df = sector_data.get(etf)
            if df is None or len(df) < 20:
                print(f"  SKIP {etf}: no data")
                continue
            close = df["Close"]
            volume = df["Volume"]
...
async def fetch_and_analyze_stocks():
    ...
    if not settings.FMP_API_KEY:
        ...
        return []
    print("=" * 50)
    print("STARTING STOCKS REFRESH (FMP)")
    print("=" * 50)
    sector_scores = {}
    ...
    async with httpx.AsyncClient(timeout=60, follow_redirects=True) as client:
        for sector_code, tickers in SECTOR_STOCKS.items():
            print(f"\n--- {sector_code}: {tickers} ---")
            batch_data = await fetch_fmp_batch(client, tickers)
            await asyncio.sleep(0.5)
            print(f"  Got: {list(batch_data.keys())}")
